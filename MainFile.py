...
path = "./initial_configuration_information/"
...
L_y = 20 * np.sqrt(3)*np.sqrt(3)/2*factor
L_z = 0
...
total_txt = 4 * saveResolutionFactor * 10
...
for i in range(int(d_pasos * total_txt)-1):
    ...
    if c == 0 :
        ...
        for k in range(n_vertices):
            ...
            for c in t[k].cells:
                cells.append(c)
            save_adjs_tipo1.append((np.array(adj).flatten()).tolist())
            save_cells_tipo1.append((np.array(cells).flatten()).tolist())

        np.savetxt('current_topology_file.txt', np.c_[nv_tipo1,save_adjs_tipo1, save_cells_tipo1], fmt='%1.0f')
